...
2. 自動同步每個已授權 Master 的最近 24 小時；App 前景每 30 秒下載增量。切到地圖仍會同步。
...
排程在 App 層執行，不依賴雲端頁。前景每 30 秒觸發，上一輪未完成就略過；切到背景取消下載，回到前景立即補下載。斷網失敗下個週期重試。每輪自動同步最多執行 120 秒，逾時保留已儲存批次，下輪繼續。手動下載會先取消並等待自動下載停止，兩者不重疊。登出取消同步並清除該手機的 Session。

5 分鐘補查只涵蓋有限的延遲寫入；雲端若回填更舊的時間，需手動下載該日期補齊。假設雲端 `received_at` 為新增時的接收時間，手機時鐘正確。

## 範圍與限制

- 不在鎖屏或背景持續下載；登入後可以離線查看已下載的快取。
...
```powershell
npm.cmd test -- --runInBand __tests__/Cloud.test.js __tests__/CloudScreen.test.js __tests__/CloudSync.test.js
```

實機驗收：已授權帳號讀取對應 Master 全部 Slave；無授權帳號下載為空；相同範圍下載兩次筆數不增加；斷網後重新讀取本機資料；取消後重試；登出與換帳號不顯示前一帳號資料。

同步驗收：登入後自動下載；切到地圖超過 30 秒後查看新資料；殺掉 App 重開不需輸入密碼並補下載；背景不新增下載；斷網再連線後 30 秒內重試；手動下載舊歷史不倒退自動進度。Android 新增原生套件，需要重新建置安裝 APK；iOS 需更新 Pods 並重建。
