...
export default function HistoryCursor({ tracks, mapRef, revision, width, height, top, bottom,
  hidden = false, selection: savedSelection, onSelectionChange }) {
      ...
      try {
        ...
        for (const track of tracksRef.current) {
          ...
          for (const segment of track.segments) {
            const projected = await Promise.all(segment.map(async p => ({ ...p,
              ...await mapRef.current.pointForCoordinate({ latitude: p.latitude, longitude: p.longitude }) })));
            if (!alive) return;
            segments.push(projected);
          }
          result.push({ ...track, segments });
        }
  ...
  const responder = useMemo(() => PanResponder.create({
    ...
    onPanResponderRelease: () => { dragStart.current = null; setDragging(false); },
    onPanResponderTerminate: () => { dragStart.current = null; setDragging(false); },
  ...
  return <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
    {box && <>
      ...
      <View pointerEvents="none" style={[styles.label, { left: box.x, top: box.y, width: box.width, height: box.height }]}>
        <Text style={styles.name}>{selection?.name || track?.name} · {dragging ? '拖動中' : selection ? '已固定，可再拖動' : '拖動三角形'}</Text>
    ...
    <View collapsable={false} pointerEvents="box-only" testID="history-cursor-handle" accessibilityLabel={`歷史游標 ${new Date(chosen.time).toLocaleString()}`} {...responder.panHandlers}
